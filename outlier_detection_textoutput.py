...
#     #Do some post - processing operations to have the ROI_ID as integers
    ...
    #Set dataframe index as ROI_IDs
    dfnew['new_ind'] = unique_roi
    dfnew.set_index('new_ind',inplace=True)
    
    #This takes all columns except the ROI_ID Column
    roidf = dfnew.loc[:, df.columns != 'ROI_ID']
    
    #Return the (index,column_name) pair of all outlier points
    outlier_list = list(roidf[roidf > 0].stack().index)
    
    #Write to file 
    with open(allFiles_new[i],'w') as file:
